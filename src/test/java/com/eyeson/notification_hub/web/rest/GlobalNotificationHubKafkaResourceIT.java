...
import com.eyeson.notification_hub.config.KafkaProperties;
...
class GlobalNotificationHubKafkaResourceIT {
    ...
    void setup() {
        ...
        GlobalNotificationHubKafkaResource kafkaResource = new GlobalNotificationHubKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/global-notification-hub-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/global-notification-hub-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
